...
export default function Sidebar({ trip, days, activeDayIndex, dayPhotos, onSelectDay, onClearDay, onPhotoClick }) {
  return (
    <aside className="w-full md:w-80 flex flex-col h-full bg-forest-800 md:border-r border-forest-600 grain-overlay">
      {/* Header */}
      ...
      {/* Day list — pb-14 on mobile so content clears the tab bar */}
      <div className="flex-1 overflow-y-auto sidebar-scroll py-3 space-y-1.5 pb-14 md:pb-3">
        <TripDescription />
        {days.map((day) => (
          <DayCard
            key={day.day_index}
      ...
      {activeDayIndex != null && dayPhotos?.length > 0 && (
        <div className="flex-shrink-0 border-t border-forest-600 mb-14 md:mb-0">
